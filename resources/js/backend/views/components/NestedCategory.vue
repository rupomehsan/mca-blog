<template>
    <li v-for="item in children" :key="item.id">
        <div class="details">
            <input
                :type="type"
                :name="type == 'radio' ? 'parent_id' : 'blog_category_id[]'"
                :class="type == 'radio' ? 'form-check-input' : ''"
                :value="item.id"
                @click="type == 'checkbox' ? setCategory(item) : ''"
                :checked="child_parent_id.includes(item.id)"
            />
            <div class="title">
                {{ item.title }}
            </div>
            <div
                class="collpse"
                v-if="item.child_categories?.length"
                @click="showSubCategory"
            >
                <i class="fa fa-minus fa-plus"></i>
            </div>
        </div>
        <ul class="list" v-if="item.child_categories?.length">
...
export default {
    props: ["children", "child_parent_id", "type"],
